...
class List_View_Inscrit_Quart extends WP_List_Table{
    ...
    function column_codeins($item)
    {
        ...
        $menu_page_formulaire = get_site_url()."/page/formulaire/";
        $menu_page_remove = menu_page_url(PREFIX_PLUGINS_NAME."_remove_to_phase", false );

        $actions = array(
            'create' => sprintf('<a href="' .$menu_page_formulaire. '%s" target="_blank">%s</a>',$item["codeins"], 'Imprimer ' )
        );
        ...
        return sprintf('%s %s ',
            '<a href="'.$menu_page.'&id='.$item["id"].'&return=2">'.$item['codeins'].'</a>',
    ...
    {
        if($item['etape'] == 3){
            $passage = 'Est passe en demi final';
        }
        elseif($item['etape'] <=2){
            $passage = 'Passe en demi final';
            return sprintf('%s',
                '<a href="'.menu_page_url(PREFIX_PLUGINS_NAME."_add_to_phase", false ).'&id='.$item['id'].'">'.$passage.'</a>'
            );
        }elseif($item['etape'] >= 4){
                $passage = 'Est passe en final';
        }
        return sprintf('%s',
            ''.$passage.''
        );
    ...
    function column_vote($item)
    {
        ...
        $vote = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as nbr FROM $table_vote WHERE etape = %d AND idcandidat = %d", 2, $item['id']));
    ...
    function get_bulk_actions()
    {
        global $wpdb;
        $table_phase = $wpdb->prefix. 'miss_phase';
        $phase = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_phase WHERE active = %d", 1), ARRAY_A);
        if($phase['etape'] <=2){
            $actions = array(
                'enlever' => 'Enlever'
            );
    ...
    function process_bulk_action()
    {
        ...
        if('enlever' === $this->current_action()){
            $ids = isset($_REQUEST['id']) ? $_REQUEST['id'] : array();
            if(is_array($ids)){
                $ids = implode(',', $ids);
                $wpdb->query('UPDATE '.$table_name.' SET etape = 1 WHERE id IN('.$ids.')');
                echo '<div id="message" class="updated"><p> Candidats retires de la phase avec succes</p></div>';
            }
        }
    }
    ...
    function prepare_items($search = NULL, $filtre = NULL) {
        ...
        if(!empty($search)){
            ...
            $total_items = $wpdb->get_var("SELECT COUNT(id) FROM $table_name WHERE (codeins LIKE '%%".$search."%%' OR nom LIKE '%%".$search."%%' or prenom LIKE '%%".$search."%%') AND etape >= 2");
            $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE (codeins LIKE '%%%s%%' OR nom LIKE '%%%s%%' or prenom LIKE '%%%s%%') AND etape >= 2 ORDER BY $orderby $order LIMIT %d OFFSET %d", $search, $search, $search, $per_page, ($paged*$per_page)), ARRAY_A);
...
//        }
            ...
            // will be used in pagination settings
            $total_items = $wpdb->get_var("SELECT COUNT(id) FROM $table_name WHERE etape >= 2");
            $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE etape >= 2 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, ($paged*$per_page)), ARRAY_A);
