...
class List_View_Inscrit_Demi extends WP_List_Table{
    ...
    function column_codeins($item)
    {
        ...
        return sprintf('%s %s ',
            '<a href="'.$menu_page.'&id='.$item["id"].'&return=3">'.$item['codeins'].'</a>',
    ...
    function column_vote($item)
    {
        ...
        $vote = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as nbr FROM $table_vote WHERE etape = %d AND idcandidat = %d", 3, $item['id']));
    ...
    function prepare_items($search = NULL, $filtre = NULL) {
        ...
        if(!empty($search)){
            ...
            $total_items = $wpdb->get_var("SELECT COUNT(id) FROM $table_name WHERE (codeins LIKE '%%".$search."%%' OR nom LIKE '%%".$search."%%' or prenom LIKE '%%".$search."%%') AND etape >= 3");
            $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE (codeins LIKE '%%%s%%' OR nom LIKE '%%%s%%' or prenom LIKE '%%%s%%') AND etape >= 3 ORDER BY $orderby $order LIMIT %d OFFSET %d", $search, $search, $search, $per_page, ($paged*$per_page)), ARRAY_A);
...
//        }
            ...
            // will be used in pagination settings
            $total_items = $wpdb->get_var("SELECT COUNT(id) FROM $table_name WHERE etape >= 3");
            $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE etape >= 3 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, ($paged*$per_page)), ARRAY_A);
